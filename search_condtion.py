...
def search_conditions(input_string):
    ...
    """
    pattern1 = re.compile(r'\b\w*sh\b')  # Ends with "sh"
    pattern2 = re.compile(r'\bsh\w*\b')  # Starts with "sh"
    pattern3 = re.compile(r'\bvi\w*li\b')  # Starts with "vi" and ends with "li"

    match1 = re.findall(pattern1, input_string)
    match2 = re.findall(pattern2, input_string)
    match3 = re.findall(pattern3, input_string)
